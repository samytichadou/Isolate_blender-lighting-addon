...
class IsolateLightAddonPrefs(bpy.types.AddonPreferences):
    ...
    prefs_offset_shortcut = bpy.props.BoolProperty(name="Viewport Offset Shortcut", default=True, description="If enabled, Ctrl+Alt on the Viewport GUI will move it to the mouse cursor")
    ...
    def draw(self, context):
        layout = self.layout
        layout.prop(self, "prefs_offset_shortcut")
        ...
        if winman.isolatelight_unrendered_modal==True:
            if len(hidden)!=0:
                for n in hidden:
                    ...
                    if mx>15+offx and mx<int(40*multsi)+offx and my>base and my<=base+20:
                    ...
                    if mx>=int(40*multsi)+offx and mx<int(65*multsi)+offx and my>base and my<=base+20:
                        bgl.glColor4f(*surlign)
                        for n2 in lights:
                            if n==n2:
                                hover=lamps.index(n2)
                    ...
                    if mx>=int(65*multsi)+offx and mx<int(150*multsi)+offx and my>base and my<=base+20:
                        bgl.glColor4f(*surlign)
                        for n2 in lights:
                            if n==n2:
                                hover=lamps.index(n2)
...
#            self.hover2=''
    ...
    if chks==1 or winman.isolatelight_on_off==True:
        ...
        if mx>15+offx and mx<int(150*multsi)+offx and my>base and my<=base+20:
            bgl.glColor4f(*surlign)
            self.hover2='isolate'
            hover=-1
        else:
    ...
    if winman.isolatelight_help_modal==True:
        ...
        if shortcut==True:
            ...
            blf.position(font_id, 15+offx, base, 0)
            blf.size(font_id, int(13*(size/10)), 72)
            blf.draw(font_id, "Ctrl + Shift to Move texts")
...
class IsolateLightModalDraw(bpy.types.Operator):
    ...
    regionwdt = bpy.props.IntProperty()
    _timer = None
                ...
                if self.hover2=='isolate':
                    ...
                    for ob in bpy.context.scene.objects:
                        ...
                        if ob.name==self.hover2:
                            ...
                            bpy.context.scene.objects.active = ob
        if shortcut==True:                
            if event.ctrl==True and event.alt==True and event.shift==False:
                if context.user_preferences.system.use_region_overlap==True:
                    winman.isolatelight_modal_offx=event.mouse_region_x-15-self.regionwdt
                    winman.isolatelight_modal_offy=event.mouse_region_y-80
                else:
                    winman.isolatelight_modal_offx=event.mouse_region_x-15
                    winman.isolatelight_modal_offy=event.mouse_region_y-80
...
def isolate_menu_draw(self, context):
    ...
    if winman.isolatelight_modal_working==True:
        ...
        row2.scale_x = 0.5
        row2.prop(winman, 'isolatelight_modal_offx', text='')
        row2.prop(winman, 'isolatelight_modal_offy', text='')
...
def register():
    ...
    bpy.types.WindowManager.isolatelight_modal_offx = bpy.props.IntProperty(description="X Offset of the Viewport GUI")
    bpy.types.WindowManager.isolatelight_modal_offy = bpy.props.IntProperty(description="Y Offset of the Viewport GUI")
